...
# ============================================================
# set_page_config (PREMIÈRE COMMANDE)
# ============================================================
st.set_page_config(
...
section.main p, section.main label {
    color: #000000;
}
...
def load_models():
    ...
    try:
        ...
        models['lasso'] = joblib.load('models/lasso.pkl')
        models['features'] = joblib.load('models/features_names.pkl')
...
def load_and_clean_data():
    ...
    try:
        ...
        # Garder Date pour les graphiques temporels
        # df = df.drop(columns=['Date'])  # NE PAS SUPPRIMER
...
with st.sidebar:
    ...
    st.markdown("<h1 style='color:#0071CE;'>Walmart Dashboard</h1>", unsafe_allow_html=True)
...
if page == "Accueil":
    ...
    st.info("""
    💡 **Dans ce projet:**  
    - Random Forest a montré d'excellentes performances (R² = 0.7448).  
    - Dans un contexte professionnel, je recommanderais de tester également XGBoost et LightGBM.
    """)
...
# ============================================================
    ...
    if st.button("🚀 Prédire", type="primary", use_container_width=True):
        ...
        prediction = max(prediction, 200000)
...
# ============================================================
    ...
    # Graphique 3
        ...
        with col1:
            ...
            ax1.set_title('Tendance du chômage par magasin')
            ax1.set_xlabel('Date')
            ax1.set_ylabel('Chômage (%)')
            ax1.legend()
            ax1.grid(alpha=0.3)
        ...
        with col2:
            ...
            ax2.set_title('Tendance des ventes par magasin')
            ax2.set_xlabel('Date')
        ...
        st.write(f"📊 Corrélation Year vs Unemployment: **{df_clean['Year'].corr(df_clean['Unemployment']):.3f}**")
        st.write(f"📊 Corrélation Year vs Weekly_Sales: **{df_clean['Year'].corr(df_clean['Weekly_Sales']):.3f}**")
    ...
    # Graphique 4
        ...
        for store in df_clean['Store'].unique():
            ...
            if len(df_store) > 5:
                ...
                correlations.append({'Store': int(store), 'Correlation': corr})
        ...
        st.write(f"✅ **Corrélation POSITIVE:** {len(corr_positives)} magasins → {corr_positives['Store'].tolist()}")
        st.write(f"✅ **Corrélation NÉGATIVE:** {len(corr_negatives)} magasins → {corr_negatives['Store'].tolist()}")
        ...
        ax.set_ylabel("Store")
        ax.set_title('Corrélation par magasin (vert = positif, rouge = négatif)')
...
st.markdown(
    "<p style='text-align: center; color: gray;'>Projet pour la certification BLOC 3 | Jedha Bootcamp</p>",
    unsafe_allow_html=True
)